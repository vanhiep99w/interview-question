...
Server phải giữ thêm file descriptor, session state, heartbeat và outbound buffer. Client cũng parse cùng payload nhiều lần. Nếu reconnect không có jitter, các tab của hàng nghìn users còn có thể tạo một reconnect storm cùng lúc.
