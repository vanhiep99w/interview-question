...
public class BulkMergeService {
    ...
    public void mergeFromStaging() throws Exception {
        ...
        while (offset < totalRows) {
            ...
            if (batchNum % 10 == 0) {
                log.info("Progress: {}/{} rows processed ({:.1f}%)",
                    offset, totalRows, 100.0 * offset / totalRows);
            }
...
> ```sql
> DELETE FROM orders_staging a USING orders_staging b
> WHERE a.ctid < b.ctid AND a.order_id = b.order_id;
> -- Hoặc dùng DISTINCT ON:
...
> ON CONFLICT (order_id) DO UPDATE SET ...
> RETURNING order_id,
>     (xmax = 0) AS is_insert,  -- xmax=0 → mới insert; xmax>0 → update
>     (xmax != 0) AS is_update;
> ```
