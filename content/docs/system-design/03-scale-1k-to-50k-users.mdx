...
Rule of thumb: `connections = (core_count * 2) + effective_spindle_count`. Với db.r6g.xlarge (4 vCPU): ~10 connections optimal per pool. 5 app pods → tổng 50 connections — PostgreSQL default max_connections = 100 nên vẫn thoải mái. Quan trọng: nếu cần tăng thêm, dùng PgBouncer connection pooling thay vì tăng `max_connections` — mỗi PostgreSQL connection tốn ~10 MB RAM, 500 connections = 5 GB chỉ cho connection overhead.

### 2. Cache invalidation xử lý như thế nào khi user update data?

Ba chiến lược phổ biến: (1) **TTL-based** — set TTL 60s, chấp nhận stale data tối đa 60s, đơn giản nhất và đủ cho hầu hết use case; (2) **Write-through** — write vào DB đồng thời update cache, consistent nhưng phức tạp hơn; (3) **Cache-aside + evict on write** — write thì xoá cache entry, request tiếp theo cache miss và rebuild. Phổ biến nhất là (3) — dùng `@CacheEvict` trong Spring Boot. Chọn chiến lược nào phụ thuộc vào mức độ stale data chấp nhận được — product listing stale 60s OK, account balance thì không.

### 3. Nếu tiếp tục scale lên 500K users thì bước tiếp theo là gì?

CDN cho static assets (JS, CSS, images) → giảm bandwidth và latency đáng kể. Database: thêm read replica thứ 2, partitioning bảng lớn (orders by month). Caching: nâng Redis lên cluster mode để sharding data qua nhiều node. Nếu team > 20 engineers → bắt đầu tách 1-2 bounded context thành service riêng (payment, notification) — nhưng vẫn không phải "full microservices". Mỗi bước phải có data justify: đo bottleneck trước khi thêm infra.

### 4. Làm sao biết monolith đã đến giới hạn và cần tách service?

Hai tín hiệu chính: (1) **Organizational** — team > 30 engineers, merge conflict liên tục, deploy phải coordinate quá nhiều team, release cycle chậm vì phụ thuộc chéo; (2) **Technical** — có component cần scale khác biệt hoàn toàn (image processing cần GPU mà API thì không, hoặc 1 module chiếm 80% CPU). Nếu chưa có cả 2 tín hiệu → monolith vẫn là lựa chọn đúng. Đừng tách service vì "best practice" — tách vì pain thật.

---

## Xem thêm

- [Cluster, Sharding, Replication, Primary/Replica là gì?](/system-design/01-distributed-system-terms)
