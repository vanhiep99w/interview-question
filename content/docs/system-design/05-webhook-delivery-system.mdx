...
Một câu trả lời Senior nên chốt contract trước khi vẽ architecture.

#### Functional requirements

```txt
- Customer tạo, cập nhật, disable endpoint.
- Customer chọn event types muốn subscribe.
- Hệ thống tạo immutable event payload theo schema version.
- Giao event bất đồng bộ qua HTTPS POST.
- Retry lỗi tạm thời theo policy có giới hạn.
- Hiển thị delivery và attempt history.
- Cho phép resend một event hoặc replay theo khoảng thời gian.
- Rotate signing secret mà không gián đoạn delivery.
- Có thể pause endpoint lỗi và thông báo customer.
```

#### Non-functional requirements

```txt
- Business request không chờ customer endpoint.
- Không mất event đã commit business transaction.
- Delivery semantics: at-least-once.
- Không yêu cầu global ordering.
- Một endpoint chậm không ảnh hưởng endpoint khỏe.
- Có backpressure; không dùng retry để tự DDoS receiver.
- Payload được ký, truyền qua TLS và có replay protection.
- URL customer-controlled không được truy cập internal network.
- Có audit, metrics, logs và runbook.
```

#### Câu hỏi cần hỏi Product

```txt
- 10 triệu là events trước hay deliveries sau fan-out?
- Peak/average ratio và growth 12 tháng?
- Payload trung bình/p99, có file lớn không?
- Event nào cần ordering?
- First-attempt latency SLO là bao lâu?
- Retry window: vài giờ, một ngày hay ba ngày?
- Customer có được cấu hình retry policy/rate không?
- Retention và replay window bao lâu?
- Có PII/financial data trong payload không?
- Multi-region là availability requirement hay chỉ giảm latency?
```

Không có các câu trả lời này, mọi batch size, timeout, retry count và partition count chỉ là đoán.
