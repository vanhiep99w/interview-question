...
- [Redis vs Memcached — caching strategy](/caching/01-redis-vs-memcached) — Scenario 4 (Redis Eviction Storm) liên quan trực tiếp đến caching strategy.
- [Heap vs Off-Heap Memory trong Java](/java/01-heap-vs-off-heap-vs-stack) — Scenario 1 (OOMKilled) cần hiểu JVM memory để debug root cause.
- [Scale từ 1,000 lên 50,000 users](/system-design/02-scale-1k-to-50k-users) — Monitoring và alerting cho hệ thống production.
